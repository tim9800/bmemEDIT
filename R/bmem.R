...
bmem.mi.cov<-function(x, m=10, moment=FALSE){
  # Initialization
  data <- x[,1:24]
  mi.data.ini <- mice(data, m = 1, maxit = 0, print = F)
  pred <- mi.data.ini$pred
  pred[c('BRIEF_Inhibit_Pre', 'BRIEF_EmotionalControl_Pre', 'BRIEF_Initiate_Pre', 'BRIEF_PlanOrganize_Pre', 'BRIEF_OrganizationMaterials_Pre', 'BRIEF_WorkingMemory_Pre', 'BRIEF_SelfMonitor_Pre', 'BRIEF_TaskMonitor_Pre', 'BRIEF_Shift_Pre'), 'BRIEF_GEC_Pre'] <- 0
  pred[c('SRS_RAW_Score_Pre_Comm', 'SRS_RAW_Score_Pre_Motiv', 'SRS_RAW_Score_Pre_Cog', 'SRS_RAW_Score_Pre_Aware'), 'SRS_RAW_Total_Pre_SCI'] <- 0
  meth <- mi.data.ini$meth
  meth["BRIEF_GEC_Pre"] <- "~ I(BRIEF_Inhibit_Pre + BRIEF_EmotionalControl_Pre + BRIEF_Initiate_Pre + BRIEF_PlanOrganize_Pre + BRIEF_OrganizationMaterials_Pre + BRIEF_WorkingMemory_Pre + BRIEF_SelfMonitor_Pre + BRIEF_TaskMonitor_Pre + BRIEF_Shift_Pre)"
  ...
  mi.data<-mice::futuremice(data = data,
                          m = m,
                          meth = meth,
                          pred = pred,
                          maxit = 10,
  ...
  mi.data.long.t <- mi.data.long %>%
    group_by(.imp) %>%
    # Creating Cowat Z score
    mutate(across(.cols = c(Letters_raw_YMH, Animals_raw_YMH), 
                  .fns = list(mean = ~mean(., na.rm = T)))) %>%
    mutate(across(.cols = c(Letters_raw_YMH, Animals_raw_YMH), 
                  .fns = list(sd = ~sd(., na.rm = T)))) %>%
    mutate(ZLetters = (Letters_raw_YMH - Letters_raw_YMH_mean) / Letters_raw_YMH_sd) %>%
    mutate(ZAnimals = (Animals_raw_YMH - Animals_raw_YMH_mean) / Animals_raw_YMH_sd) %>%
    mutate(ZCOWAT = (ZLetters + ZAnimals)/2) %>%
    # Converting TMT to inverse times
    mutate(TMT_B_inv = 1/TMT_B_raw_YMH) %>%
    # Converting BRIEF Raw to T scores
    bmemTOOLS::BRIEF_convert_T() %>%
    # Converting SRS Raw to T scores
    bmemTOOLS::SRS_convert_T() %>%
    # Removing unneeded columns
    select(-c("Letters_raw_YMH_mean":"ZAnimals"))
  
  mi.data <- as.mids(mi.data.long.t)
  mi.data <- complete(mi.data, action = "all", include = F)
